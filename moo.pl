		...
		if ($#files>-1 && !$opt_n){#runs if there are files and opt_n is not specified
			for (my $i=0;$i<=$#files;$i++){
				...
				if ( -e "$db_dir/$db_file.idx"){#indexing available so you do not need a lot of space
					$int="16";$idxd=1;
					my $batchsize=($db_file=~/(wgencode|dfam|rptmask|avsnp)/i)?'1000':'100k';
				...
				if (($db_file=~/(avsnp14\d+|1000g|clinvar|snp1[23]\d|regulome\d+|avsift|ljb|vcf|cg69|siftv63|hapmap|nci60|provean|esp6500|ExAC|(ALL|AFR|AMR|EAS|EUR|SAS).sites)/i)){#large files with index
