...
if ( `uname -a`=~/(fr-s-hpc-head-1|avia)/i && $#ARGV>-1 && join("",@ARGV)!~/\-g/){
	...
	exec ("$qsub -N PBS_$dir -l nodes=1:ppn=1 PBS.MASTER\n");#changed 20150323
	exit;
}else{
...
use lib "$FindBin::Bin/";
use ABCC2;
use vars qw($opt_j $opt_C $opt_W $opt_I $opt_E $opt_R $opt_A $opt_G $opt_w $opt_z $opt_F $opt_Q $opt_x $opt_m $opt_H $opt_f $opt_o $opt_i $opt_d $opt_T  $opt_h $opt_t $opt_c $opt_g $opt_n $opt_l $opt_b $opt_B $opt_k $opt_r $opt_s $opt_N $opt_v $opt_S $opt_p $opt_P $opt_V $opt_e $opt_M);
 getopts("f:o:c:d:i:T:t:l:B:k:p:m:F:A:j:P:rvRzsbcehxgnHEINSwQWIMGCeV");
 umask(0000);
my $usage=qq{
	$0 -i <input filename>
	...
	[OPTIONAL]
		...
		-I <do NOT run NMD Indel script even though SIFT is specified in -f option>
	...
	[CONDITIONAL]
		...
		-e <if specified, uses SEPARATE option for gene annotations>  This WILL alter users' standard outputs!!!!
	[RERUNNING OPTIONS]
		...
		-M <if specified, then if an annotation file (e.g ANNOVAR.input_000.hg19_avsnp) exists, it will RERUN the annotation.  >
				By default, it will not rerun an annotation if it exists so it will save some time for reruns
				If you wish to overwrite existing annotations, then specify this option or delete the annotation in the directory
...
my $exceptions='(\_af$|PTM$|.*funseq.txt|.*uniprot.txt|.*FlankingSequence|.*funseq2.txt|mutAligner|sift4G|avp_[\w\-]+)';##.*Encode.*.txt|
...
my $annovar="perl $bin/annotate_variation_ABCC2IP.pl ";##halloween
if ($opt_V){
...
if (!$opt_P) {##if not specified on command line, check if it is in the database file
	my @protDBs= join ("",@files)=~/$opt_o\_(dbPTM|mutAligner|PhosphoSitePlus|PTM|avp_[\w\-]+)/g;
	$opt_P.= join (",",@protDBs);
...
#			system ("rm $opt_i\\_* -f\n");
	...
	for (my $k=0;$k<$ct;$k++){
		...
		my $pass;
		if ($opt_e){
			$pass = checkABCCLine("$opt_i\_$k");
			if ($pass==0){
				warn "[WARN] You cannot use this option for AVIA unless you run convert2ANNOVARIP.pl on your input file first.  AVIA is ignoring --separate option...\n";
			}else{
				$ensembl.=" --separate ";
			}
		}
...
my $last_file=($ct==0)?"$opt_i\_".sprintf("%03d",0):"$opt_i\_".sprintf("%03d",$ct-1);#off by one bc we start at 0
			...
			for (my $i=0;$i<=$#files;$i++){
				...
				if ($db_file=~/$exceptions/i ){
					...
					# if ( !$opt_W || ($files[$i]=~/encode/i && !$opt_W) || ($opt_W && $files[$i]!~/encode/i)){##commented out hv 
						print "Ran into an exception...$exception...\n";
				...
				if ( -e "$db_dir/$db_file.idx"){#indexing available so you do not need a lot of space
					$int="16";$idxd=1;
					my $batchsize=($db_file=~/(wgencode|dfam|rptmask|avsnp|regulome|hrcr|icgc)/i)?'1000':'10000';#for really big annotations
				...
				my $final="$opt_i". "_$k.$opt_o". "_$db_file";
				if (!$opt_M && -e "$final"){
					##double check that errors do not exist..
					if (`grep '\\bERR\\b' $final | wc -l `==0){
						print "\tLooks like file $final exists already and does not have any errors...skipping\n";
						next;
					}else{
						print "\tLooks like file $final exists already and but has errors\n";
					}
				}
				...
				if ($db_file=~/vcf/){
					...
					$int=8;
				}elsif (($db_file=~/(dbnsfp|avsnp14\d+|1000g|mcap|clinvar|snp1[243]\d|icgc|regulome\d+|avsift|ljb|vcf|cg69|siftv63|hapmap|nci60|rbpvar_v|provean|esp6500|kaviar|hrcr|ExAC|(ALL|AFR|AMR|EAS|EUR|SAS).sites)/i) && $db_dir=~/\/SeqIdx\/annovardb\/(human|mouse)/){#large files with index
					if ($db_file=~/mcapv1/){ $db_file.= " --force ";}
					...
					$toDo="$annovar --buildver $opt_o $opt_c --dbtype $db_file $other --force --keepline $addon $opt_i\_$k  $db_dir";
					print "Forcing filter\n$toDo\n\n";
				}elsif($other){
					$toDo=~s/--regionanno/--regionanno $other/;
					print "Added $other todo\n$toDo\n";
	...
	CHUNK: foreach (my $k=0;$k<$ct;$k++){
		...
		if (-e "$annovar_output.collapsed" || $opt_N){
			...
			if ($opt_l && -e "$annovar_output.collapsed"){#collapse the exonic and variant file beginning at $opt_l
				# die "at line 679\n";
				_collapseANNOVAR($annovar_output,$opt_l);
				...
				_collapseANNOVAR($annovar_output,0);
...
sub printPBS{
	...
	if (!$opt_g){
		...
		my $jobs=`$qsub -l nodes=1:ppn=1,walltime=290:00:00 PBS.$opt_i.$name.bat 2>&1`;
		chomp $jobs;
...
sub record{#record keeping of who uses this script
	my $status=shift;
	my $foo;
	return if ($user=~/(vuonghm|svc$)/i);#do not record web or vuonghm:wq
...
sub checkABCCLine{
	...
	my $pass=0;
	if (-e $file){
		$pass = `grep ABCC_TAG $file | wc -l`;chomp $pass;
	}
	return $pass;
}