...
if ($opt_t=~/funseq2/i){
	...
	if (! -e "$out"){
		my $cmd;$id=($id=~/^([\w\-]+)\./)?$1:$id;
		# Run FunSeq executable here if results do not exist
		if (!-e "$fs2outputdir/Output.bed" && !-e "$fs2outputdir/Output.indel.bed"){
			$cmd="$funseq2_exe $opt_o -f $opt_f -o $fs2outputdir\n";
			monitor_grid($cmd) ;
		}
		if (-e "$fs2outputdir/Output.bed"  && -e "$fs2outputdir/Output.indel.bed"){
	...
	foreach my $file (@_){
		...
		while (<FH>){
			...
			if ($headerFound){
				...
				$idx=~s/^chr//g;
				if (exists $hash{$idx}){
					print "$idx already exists" and <STDIN>;
