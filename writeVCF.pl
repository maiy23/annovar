					...
					if ($i==0){
						...
						foreach my $id (sort keys %idx){
							...
							if ($FunSeq2 && $id=~/;/ ){
								...
								# print "printing for fun:". join ("\n",@FunSeq2_arr);<STDIN>;
								for(my $j=0;$j<=$#FunSeq2_headers;$j++){
									# print "printing for fun:$FunSeq2_headers[$j]xx=$FunSeq2_arr[$j];";<STDIN>;
									$last_annot.=uc("FunSeq2_".$FunSeq2_headers[$j])."=$FunSeq2_arr[$j];" and "adding\n" if ($FunSeq2_arr[$j] ne '.');
								}
							}elsif($exac && $id=~/ExACv/i && $val=~/:/){
								my @exac_arr=split(":",$val);
								foreach my $foo (keys %exac){
									$last_annot.= uc$foo."=$exac_arr[$exac{$foo}[1]];";
								}
...
#						$debug=1;print "\t[DEBUG]advancing annot,$#multi_alleles,$i\n\t".join ("\t",@{$aref})."\n";<STDIN>;
					...
					if ($debug){print $last_annot;<STDIN>;}
					$$aref[7].=$last_annot  if ($i==$#multi_alleles);
				}else{
...
sub getIDField{
	...
	if ($opt_t=~/(rsid|append|none)/i || ($opt_t=~/user_defined/ && $opt_n)){
		...
		if ($opt_t=~/rsid/){
			$replaced_name="MOO2".$$name_href{$id+$OffbyOne};
		}elsif($opt_t=~/append/){
			$replaced_name=($curr ne '.')?$curr.";$$name_href{$id+$OffbyOne}" : $$name_href{$id+$OffbyOne};
		}elsif($opt_t=~/none/){
			$replaced_name="ABCC_".sprintf("%04d",$id);
		}elsif($opt_t=~/user_defined/){
			$replaced_name="MOO1".$$name_href{$id+$OffbyOne};
		}
