...
$outfile ||= $queryfile;
$genetype ||= 'refgene';
$genetype =~ m/^refgene|knowngene|ensemblgene$/i or pod2usage ("Error in argument: the --genetype can be 'refgene', 'knowngene' or 'ensemblgene' only");
...
sub checkFileExistence {
	my @file = ("${buildver}_refGene.txt", "${buildver}_refLink.txt", "${buildver}_refGeneMrna.fa", "${buildver}_genomicSuperDups.txt", 
		"${buildver}_phastConsElements${mceway}way.txt",
	...
	for my $i (0 .. @file-1) {
		...
		-f $dbfile or die "Error: the required database file $dbfile does not exist. Please download it first.\n";
	}
}
...
=head1 SYNOPSIS
 ...
 Optional arguments:
        ...
        -v, --verbose                   use verbose output
            ...
            --ver1000g <string>		version of 1000G data (1000g, 1000g2010, 1000g2010jul, 1000g2010nov)
