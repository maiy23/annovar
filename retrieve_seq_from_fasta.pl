...
my (%seqlen, %discordlen);		#store the length of each sequence, and the ID of sequences with discordant length
...
for my $curchr (sort keys %$allchr) {
	...
	for my $i (0 .. @{$allregion->{$curchr}}-1) {
		my ($name, $start, $end, $strand, $exonpos) = @{$allregion->{$curchr}[$i]};
...
for my $i (0 .. @$sorted_region-1) {
	...
	if ($tabout) {
		...
		print ">$name ", $discordlen{$name}?"Warning: $name occur more than once in file with discordant length. ":"", 
