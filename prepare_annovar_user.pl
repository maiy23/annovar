...
our ($dbtype, $dbfile, $outfile, $vcffile, $dbsnpfile, $buildver, $transcriptid, $cdotfile, $afstring, $acstring, $infostring, $twopos, $dbnsfpver,$forAVIA);
...
GetOptions('verbose'=>\$verbose, 'help|h'=>\$help, 'man'=>\$man, 'outfile=s'=>\$outfile, 'dbtype=s'=>\$dbtype, 'vcffile=s'=>\$vcffile,
	...
	'acstring=s'=>\$acstring, 'infostring=s'=>\$infostring, 'twopos'=>\$twopos, 'dbnsfpver=s'=>\$dbnsfpver, 'forAVIA'=>\$forAVIA) or pod2usage ();
...
sub prepareClinVarOld {
	my ($dbfile) = @_;
	open (FH, "perl $bin/../latest/annovar/convert2annovar.pl -format vcf4 -include $dbfile |") or die "Error: cannot read from dbfile $dbfile: $!\n";
...
sub prepareClinVar {
	...
	while (<FH>) {
		...
		$clndsdbid =~ s/,/\\x2c/g;
		if ($forAVIA){
			print join ("\t", @field[0..4], "CLINSIG=$clnsig;CLNDBN=$clndbn;CLNACC=$clnacc;CLSDB=$clndsdb;CLNDSDBID=$clndsdbid\n");
		}else{
