...
while (my $line=<ANNOT>){
  ...
  if ($annot[$idx_start+3] eq '-'){
    ...
    $annot_hash{$header}=join ("\t",@annot[0..($idx_start-1)]);
    # print Dumper (\%annot_hash);exit;
  }
}
close ANNOT;
...
#	next if ($dir2run!~/0001/);
  ...
  print "\tRunning $dir2run\n";
  open (OUT,"> $file_dir/$file.annovar_wrpr.output");
  print "\tprinting output to $file_dir/$file.annovar_wrpr.output\n";
  ## Add headers for VCF file:
  my $vcf_header='';
  if (-e $file){
    $vcf_header=`head -n1 $file`;chomp $vcf_header;
    $vcf_header="\t".$vcf_header;
    # if ($vcf_header=~/filter\tinfo\tformat\t(.*)/i){
    #   $vcf_header="\t$1";
    # }else{
    #   die "$vcf_header\n";
    # }
  }
  print OUT $annot_hash{"Chr\tQuery Start\tQuery End\tAllele1\tAllele2"}."$vcf_header\n";
  while (my $line=<FILE>){
  	next if ($line=~/^\s+$/);
  	$line=~s/[\r\n]//;
  	...
  	if (exists $annot_hash{$header}){
  		print OUT "$annot_hash{$header}\t$info\n";
  	}elsif ($header=~/#/){
  	}else{
      if ($file_info[3] eq '-'){
