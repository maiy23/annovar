...
my $annovar_exe="$annovar_dir".'/annovar_qsub_wrprIP.pl';
my $annovar_convert="$annovar_dir/".'convert2annovarIP.pl';
...
$cfg{'RUNNMD'}||='false';
my $runnmd;
if ($cfg{'RUNNMD'}=~/(off|0|false)/i){
	$runnmd="-I";
}
$cfg{'BUILDVER'}||='hg19';
if ($cfg{'BUILDVER'}!~/:/){
	$cfg{'BUILDVER'}=orgLookup($cfg{'BUILDVER'});
}
...
my $sfx=(exists $cfg{'SUFFIX'})?$cfg{'SUFFIX'}:".txt";
$sfx=".".$sfx if ($sfx!~/^\./);
# read directory
...
while (my $file = readdir($DIR)){
	next if ($file=~/^(\.*|PROC|$opt_c|$dbfn|log.err|.*bat|.*log|config.ini|withannot)$/);
	next if (! -f $file);
	if (!exists $files{$file}){
	   ...
	   if (! -e "$cfg{INPUT_DIRECTORY}/PROC/$files{$file}"){
	   		...
	   		SAFE_PROCESS ("perl $annovar_convert -format $cfg{INPUT_TYPE} -o ANNOVAR.input --buildver $cfg{'BUILDVER'} -allallele --includeinfo $file\n",__LINE__);#run converter
	   		chdir ($cfg{INPUT_DIRECTORY});
	   }else{
...
chdir ("All");
system ("ln -s $cfg{DATABASE_FILE} .\n");
my $whattomonitor=monitor ("perl $annovar_exe $runnmd -f $dbfn -i All.input.txt",0);#the second argument 0 tells the subroutine to return and not wait for the job to complete; returns the grid job id
##make a global bat file in case the grid job failed....
open (BAT,">$cfg{INPUT_DIRECTORY}/PROC/All/post_processing.bat") or die "Cannot open post_processing.bat\n";
print BAT qq(
#!/bin/sh
...
print BAT qq(if [ -e '$cfg{INPUT_DIRECTORY}/PROC/All/DONE.All.input.txt.anvr.wrpr' ];then
	...
	my ($dev,$server);
	if ($opt_l=~/dev/){
		$server='/mnt/webrepo/fr-s-bsg-avia-d/public/data';$dev='dev';
	}else{
		$server='/mnt/webrepo/fr-s-bsg-aviap/public/data';$dev='';
	}
	print BAT qq(if [ -e '$cfg{INPUT_DIRECTORY}/PROC/All/DONE.All.input.txt.anvr.wrpr' ];then
...
close BAT;
system ("chmod 777 $cfg{INPUT_DIRECTORY}/PROC/All/post_processing.bat\n");
...
sub orgLookup{
	...
	if (exists $buildver{$ucsc_build}){
		return $buildver{$ucsc_build};
	}else{
		return $buildver{'hg19'};
	}
}