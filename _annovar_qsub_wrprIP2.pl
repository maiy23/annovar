	...
	for (my $k=0;$k<$ct;$k++){#make pbs bat files for all of the databases
		...
		if ($#files>-1 && !$opt_n){#runs if there are files and opt_n is not specified
			for (my $i=0;$i<=$#files;$i++){
				...
				if (($db_file=~/(avsnp14\d+|1000g|clinvar|snp1[23]\d|regulome\d+|avsift|ljb|vcf|cg69|siftv63|hapmap|nci60|provean|esp6500|kaviar|hrcr|ExAC|(ALL|AFR|AMR|EAS|EUR|SAS).sites)/i)){#large files with index
