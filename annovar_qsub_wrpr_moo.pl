		...
		if ($#files>-1 && !$opt_n){#runs if there are files and opt_n is not specified
			for (my $i=0;$i<=$#files;$i++){
				...
				if ($db_file=~/vcf/){
					...
					$int=8;
				}elsif (($db_file=~/(avsnp14\d+|1000g|mcap|clinvar|snp1[243]\d|icgc|regulome\d+|avsift|ljb|vcf|cg69|siftv63|hapmap|nci60|rbpvar_v|provean|esp6500|kaviar|hrcr|ExAC|(ALL|AFR|AMR|EAS|EUR|SAS).sites)/i) && $db_dir=~/\/SeqIdx\/annovardb\/(human|mouse)/){#large files with index
